                ...
                while (true)
                {
                    ...
                    switch (poruka.Operations)
                    {
                        case Operations.Igra:
                            ...
                            {
                                lista.Add(((Info)poruka.PorukaObject).Broj);
                                Invoke(new Action(() => txtBrojevi.Text += ((Info)poruka.PorukaObject).Broj.ToString() + " "));
                                Invoke(new Action(() => txtCount.Text = lista.Count().ToString()));
                            }
                            break;
                        case Operations.ZavrsiIgru:
                            Invoke(new Action(() => btnBrisi.Enabled = false));
                            Invoke(new Action(() => lblInfo.Text = "Svi brojevi su obrisani"));
                            break;
                    }
                }
            }
